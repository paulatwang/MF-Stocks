...
stocks30 <- read.table("data30_60shift.csv", header=TRUE, stringsAsFactors=TRUE, sep=",", na.strings="NA", dec=".", strip.white=TRUE)  # READ DATA
stocks30$dt_30min <- as.POSIXct(stocks30$dt_30min, tz="EST") # CONVERT DT TO POSIX
col60 <- c("season_workday_30min", "season_month_30min","tf1_30min", "tf2_30min", "tf3_30min", "tf4_30min", "tf5_30min", "tf6_30min") 
...
omtted30 <- na.omit(stocks30_ordered)
...
stocks30_outrm_moralityonly <- outlier_rm_IQR(omtted30$morality_lag[-1], "omtted30", "morality_lag", 2.5)

stocks30.model.lm <- lm(stocks_30min_diff_ln ~ tf2_30min * morality_lag,data = omtted30)

stocks30.model.lm.foundations <- lm(stocks_30min_diff_ln ~ tf2_30min * (care_lag +fairness_lag +loyalty_lag +authority_lag +sanctity_lag), data = omtted30)

stocks30.model.lme.null <-lmer(stocks_30min_diff_ln ~ 1 + (1|day_count), data = omtted30, REML=TRUE)

stocks30.model.lme <- lmer(stocks_30min_diff_ln ~ (1 + season_intraday_30min + tf2_30min*morality_lag + (1 + season_intraday_30min | day_count)), data = omtted30, REML = TRUE)

stocks30.model.lme.foundations <- lmer(stocks_30min_diff_ln ~  (1 + season_intraday_30min + tf2_30min*(care_lag + fairness_lag + loyalty_lag + authority_lag + sanctity_lag) + ( 1 + season_intraday_30min | day_count)), data = omtted30, REML = TRUE)
